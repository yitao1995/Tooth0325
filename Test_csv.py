...
parser.add_argument('--netG', default='Checkpoint/point_netG60.pth', help="path to netG (to continue training)")
parser.add_argument('--infile', type=str, default='test_one/0002黄剑斌.csv')
parser.add_argument('--infile_real', type=str, default='test_one/0002黄剑斌.csv')
parser.add_argument('--netD', default='Checkpoint/point_netD60.pth', help="path to netD (to continue training)")
...
parser.add_argument('--point_scales_list', type=list, default=[106048, 52512, 26256],
...
""" Test data setting """
# opt.infile_real = 'test_one/0001陈雪柔.csv'
filenaam = os.path.basename(opt.infile_real)
filenaam = filenaam.split(".")[0]
""" End """

test_dset = shapenet_part_loader.PartDataset(root='./dataset/shapenetcore_partanno_segmentation_benchmark_v0/',
                                             classification=True, class_choice='Car', npoints=opt.pnum, split='test')
...
np.savetxt('test_one/' + filenaam + '_crop_ours' + '.csv', np_crop, fmt="%f,%f,%f")
np.savetxt('test_one/' + filenaam + '_fake_ours' + '.csv', np_fake, fmt="%f,%f,%f")
