...
if __name__ == '__main__':
    ...
    dset = dataloader.PartDataset(root='C:/Users/hs/Desktop/pointcloud/dataset/',
    ...
    if opt.D_choose == 1:
        for epoch in range(resume_epoch, opt.niter):
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                # Set viewpoints
                batch_size = real_point.size()[0]  # b batchsize
                #    real_center = torch.FloatTensor(batch_size, 1, opt.crop_point_num, 3)  # (b,1,512,3). # 保存裁剪点的坐标
                #    input_cropped1 = torch.FloatTensor(batch_size, opt.pnum, 3)  # (完整数据)
                # (b,2048,3). 原始点云数据的坐标，后面将裁剪掉crop_point_num个点
                ...
                #                                 key=lambda x: x[1])  # enumerate使其变成2维，x[1]第二维度
                ...
                # 被裁剪后的点云
                # scale 0
                input_cropped1 = torch.squeeze(input_cropped1, 1)  # (b,1,512,3) -> (b,512,3)
                # scale 1
