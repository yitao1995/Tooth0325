...
parser.add_argument('--netG', default='Checkpoint/point_netG200.pth', help="path to netG (to continue training)")
parser.add_argument('--netD', default='', help="path to netD (to continue training)")
parser.add_argument('--manualSeed', type=int, help='manual seed')
parser.add_argument('--drop',type=float,default=0.2)
parser.add_argument('--num_scales',type=int,default=3,help='number of scales')
parser.add_argument('--point_scales_list',type=list,default=[1024*4, 1024, 512],help='number of points in each scales')
...
if __name__ == '__main__':
    ...
    test_dset = dataloader.PartDataset( root='C:/Users/hs/Desktop/pointcloud/dataset/',
                                      classification=True, class_choice=None, npoints=opt.pnum, split='test')
    ...
    for i, data in enumerate(test_dataloader, 0):
        ...
        real_point, target, coloboma, rest  = data

        real_point = torch.unsqueeze(real_point, 1)
        batch_size = real_point.size()[0]
        real_center =torch.unsqueeze(coloboma, 1)
        input_cropped1=torch.unsqueeze(rest, 1)
        p_origin = [0,0,0]
    ...
    #    if a!=b:
            ...
            np.savetxt('test_example/crop_txt'+str(n)+'.txt', np_crop, fmt = "%f,%f,%f")
            np.savetxt('test_example/fake_txt'+str(n)+'.txt', np_fake, fmt = "%f,%f,%f")
            np.savetxt('test_example/real_txt'+str(n)+'.txt', np_real, fmt = "%f,%f,%f")
